/**
 * Lab 05 — El-Basaama (Banner Grabbing & CVEs)
 * Layout: left mission-tracker + right BannerTerminal
 */
...
export default function Lab05() {
    ...
    const handleCommandRun = (cmd: string) => {
        ...
        // Step 1: nc port 80
        if ((c.startsWith('nc ') || c.startsWith('netcat ')) && c.includes('192.168.1.5') && c.includes('80') && !steps[0].completed) { completeStep(1); return }
        // Step 2: nc port 22
                    ...
                    <div className="p-5">
                        ...
                        <div className="mb-5">
                            <div className="flex items-center gap-2 mb-1">
                                <span className="font-mono text-xs text-gray-600 bg-dark-card border border-dark-border px-2 py-1 rounded-full">Lab 05</span>
                                <span className="font-mono text-xs text-neon-amber/70 bg-neon-amber/10 border border-neon-amber/20 px-2 py-1 rounded-full">Advanced · 250 pts</span>
                            </div>
                            <h1 className="font-mono font-black text-2xl text-white">El-Basaama</h1>
                            <p className="font-mono text-xs text-neon-amber mt-0.5">Banner Grabbing & CVE Analysis</p>
                            <p className="font-cairo text-xs text-gray-500 italic mt-1">(السيرفر بيحكيلك عن نفسه — إنت بس محتاج تعرف تسمع 🏷️)</p>
                        </div>
                        ...
                        <div className="mt-5 border-t border-dark-border pt-4">
                            ...
                            <div className="space-y-1">
                                {['netcat (nc) — universal TCP/UDP Swiss army knife', 'Banner grabbing — services announce themselves', 'HTTP headers — Server, X-Powered-By leaks', 'Telnet = plaintext in 2026 is security malpractice', 'searchsploit — offline exploit-db CVE lookup', 'CVSS scores — 0-10 severity rating system', 'WhatWeb — one-shot web technology fingerprint', 'Nikto — automated web vulnerability scanner', 'Banner → version → CVE → exploit chain'].map((c) => (
